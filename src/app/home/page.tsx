...
export default async function Page() {
  const categoriesComponents = categories.map((category) => (
    <Link
      ...
      }  flex flex-row gap-2 items-center rounded-full w-40 h-16 shadow text-gray-100`}
    >
      <div
        className={`h-full aspect-square px-3 ${
          category.locations > 0 ? 'bg-secondary-200' : 'bg-gray-400'
        } rounded-full flex justify-center items-center`}
      >
        <i className={`${category.icon} mt-1 text-3xl`} />
      </div>
      <div className="justify-self-end">
        <p>{category.name}</p>
      </div>
      <div className="absolute -top-2 -right-2 bg-white rounded-full size-8  align-bottom p-1 shadow-md text-gray-500  text-center text-md">
        {category.locations}
      </div>
    </Link>
  ));
    ...
    <Fragment>
      ...
      <div className="container mx-auto">
        ...
        <div className="z-10 w-full mt-6">
          <div
            className="bg-secondary rounded-full shadow-lg w-40 mx-auto max-w-xl flex flex-row py-2 px-3 items-center 
      ...
      focus-within:bg-white focus-within:text-gray-400 focus-within:w-80 "
          >
            ...
            <input
              type="search"
              placeholder="Пошук"
              className="w-full bg-inherit px-4 py-2 text-lg text-white placeholder:text-gray-100 
